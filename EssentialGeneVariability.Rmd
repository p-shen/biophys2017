...
```{r}
gx.tissue <- as.data.frame(t(gx[,3:ncol(gx)]))
colnames(gx.tissue) <- gx$Description
gx.tissue$SAMPID<- names(gx)[3:ncol(gx)]
gx.tissue <- attr %>% select(SAMPID,SMTS,SMTSD) %>% right_join(gx.tissue)
```
```{r}
eg <- data.table::fread("essential_genes.csv",drop = 1)
neg <- data.table::fread("nonessential_genes.csv",drop = 1)
gx.eg.bysite <- gx.tissue %>% group_by(SMTS) %>% select(one_of(eg$x)) 
gx.neg.bysite <- gx.tissue %>% group_by(SMTS) %>% select(one_of(neg$x))
```
...
In the following, I pick DDX11L1 as example.
```{r}
# select one gene with the tissue types and its expressions
ess_gene_tissue_exp <- gx.tissue[, c(2, 4)]
