...
```{r}
tissue_list <- unique(gx_by_gene$SMTS)
...
get_pvalue_table <- function(gene,filter.zero = FALSE, normalized = FALSE){
  # Get the expression data according selected gene
  gene_id <- match(gene,colnames(gx_by_gene))
  ess_gene_tissue_exp <- gx_by_gene[, c(2, gene_id)]

  # Whether filter out expression with 0  
  if(filter.zero == TRUE){
    ess_gene_tissue_exp <- ess_gene_tissue_exp[which(ess_gene_tissue_exp$DDOST!=0),]
  }
  
  # Make all the value lie between 0 - 1
  if(normalized == TRUE){
    ess_gene_tissue_exp <- ess_gene_tissue_exp %>% group_by(SMTS) %>% mutate(DDOST= normalize(DDOST))
    ess_gene_tissue_exp <- as.data.frame(ess_gene_tissue_exp)
  }
  
  # Get pvalue list across all the cell lines
  tissue_pvalue_list <- c()
  for(tissue in tissue_list){
  # select one cell line as one group, and the remaining cell lines as another
  target_gene_tissue_exp <- ess_gene_tissue_exp[which(ess_gene_tissue_exp$SMTS==tissue),2]
  remaing_gene_tissue_exp <- ess_gene_tissue_exp[which(ess_gene_tissue_exp$SMTS!=tissue),2]
  
  # Perform t.test of 2 groups
  pval <- t.test(target_gene_tissue_exp, remaing_gene_tissue_exp)$p.value
  tissue_pvalue_list <- append(tissue_pvalue_list,pval)
  tissue_pvalue_list
}
  # Store the results as a data.frame
  tissue_ttest_results <- data.frame(tissue = tissue_list, pvalue = tissue_pvalue_list)
  tissue_ttest_results
}

```

```{r}
# get t.test results of gene
get_ttest_results <- function(gene){
  ttest_results <- get_pvalue_table(gene)
  ttest_results_filtered <- get_pvalue_table(gene,filter.zero = TRUE)
  ttest_results_normalized <- get_pvalue_table(gene,normalized = TRUE)

  # Join all the results in one table
  ttest_results <- ttest_results %>% left_join(ttest_results_filtered,by='tissue') %>% inner_join(ttest_results_normalized,by='tissue') 
  # Rename all column names
  names(ttest_results) <- c("tissue","pvalue","pvalue.filtered","pvalue.normalized")
  ttest_results
}

# Write the results to files
output_ttest_results <- function(gene_list,type){
  for(gene in gene_list){
    tr <- get_ttest_results(gene)
    
    # Create subfolders first before creating the file
    dir.create((paste0("../ttest_results/")),showWarnings = FALSE)
    dir.create(paste0("../ttest_results/",type),showWarnings = FALSE)
    filename <- paste0("../ttest_results/",type,"/",gene,"_ttest_results.csv")
    
    write.csv(tr, file=filename,row.names = FALSE)

  }
}
output_ttest_results("DDOST",type = "essential_gene")
#output_ttest_results(names(matched_ess),type = "essential_gene")
#output_ttest_results(names(matched_non_ess),type = "non_essential_gene")
