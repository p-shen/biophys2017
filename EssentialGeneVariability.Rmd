...
get_pvalue_table <- function(gene,filter.zero = FALSE, normalized = FALSE){
  ...
  if(filter.zero == TRUE){
    ess_gene_tissue_exp <- ess_gene_tissue_exp[which(ess_gene_tissue_exp[,gene]!=0),]
  }
  
  # Make all the value lie between 0 - 1
  if(normalized == TRUE){
    for(tissue in tissue_list){
      ess_gene_tissue_exp[which(ess_gene_tissue_exp$SMTS==tissue),2] <- normalize(ess_gene_tissue_exp[which(ess_gene_tissue_exp$SMTS==tissue),2])
...
get_ttest_results <- function(gene){
  ...
  ttest_results <- ttest_results %>% left_join(ttest_results_filtered,by='tissue') %>% left_join(ttest_results_normalized,by='tissue') 
...
output_ttest_results("DHDDS",type = "essential_gene")
