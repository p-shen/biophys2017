...
noness_gene_list <-non_ess_genes_in_go
...
ggplot(gx_var, aes(x = gene, y = var, fill = type)) + geom_bar(stat= 'identity') + ggtitle("Genes in RNA Metabolic Process Pathway") + theme(axis.text.x=element_text(angle=90, hjust=1,size = 6)) + ylab("Variation")


ess_var <- gx_var[gx_var$type=="essential",]$var
noness_var <- gx_var[gx_var$type=="non-essential",]$var
...
```{r}
corrplot(cor(vio_city,use = 'complete.obs'),type = "lower",diag = FALSE)
```