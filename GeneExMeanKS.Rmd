...
ess_gene_list <- sample(names(ess_gx[-1]),3)
noness_gene_list <-sample(names(noness_gx[-1]),3)
...
# Plot the variation of each gene devided by mean
```{r}
gx_varbymean <- gx_tissue_forplot %>% group_by(gene) %>% summarise(var_bymean = sd(expression)/mean(expression)) 
gx_varbymean$type <-  ifelse(gx_varbymean$gene %in% ess_gene_list, "essential","non-essential")
ggplot(gx_varbymean, aes(x = gene, y = var_bymean, fill = type)) + geom_bar(stat= 'identity') + ggtitle(tissue)
```